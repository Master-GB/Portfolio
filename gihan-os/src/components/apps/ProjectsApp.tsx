...
const TECH_BADGES: Record<string, { color: string; bg: string }> = {
  ...
  "framer-motion": { color: "#99be91ff", bg: "rgba(122, 4, 4, 0.2)" },
...
const PROJECTS: Project[] = [
  {
    ...
    description: "GihanOS is an innovative portfolio website that mimics a desktop operating system interface.A unique OS-inspired portfolio built with Next.js, TypeScript and Tailwind CSS.",
    longDescription: "GihanOS transforms a traditional portfolio into an immersive desktop operating system experience. Built as a Next.js application, it features a complete window management system with draggable windows, taskbar, start menu, and 14 functional applications. The project integrates Three.js for an interactive 3D robot assistant, canvas-based animated space wallpapers, and a sophisticated boot sequence. It showcases full-stack development skills through complex state management, responsive design, and creative UI/UX patterns.",
    features: ["Desktop-like interface with draggable windows", "Interactive 3D robot assistant with animations", "Multiple apps: About, Projects, Skills, Resume, etc.", "Responsive design with smooth animations", "LocalStorage persistence for user preferences"],
    tech: ["Next.js", "TypeScript", "Tailwind CSS", "Three.js","framer-motion"],
    tags: ["Frontend"],
    accentColor: "#8b5cf6",
    github: "https://github.com/Master-GB/Portfolio.git",
    demo: "https://www.gihan-bandara.com/",
    gradient: "linear-gradient(135deg,#1e1b4b 0%,#312e81 50%,#1e1b4b 100%)",
    image: "/images/port.png",
    challenges: ["Implementing smooth window dragging without lag", "Building a performant window management system with state handling", "Integrating Three.js 3D graphics seamlessly with React components and state","Managing responsive layouts across different screen sizes while maintaining OS-like functionality"],
    outcomes: ["Created a unique and memorable portfolio experience", "Demonstrated next.js and Three.js skills", "Received positive feedback on the innovative design"],
  },
  {
    id: "p2",
    title: "SmartHealth",
...
/*
  NOTE ON THE FIX:
  - The overlay used to be `position: fixed; inset: 0`, which places it relative
    to the browser viewport — so it covered the entire desktop, not just this
    app's window. It's now `position: absolute` via the `.project-modal-overlay`
    class, which is scoped to `.projects-app-root` (the nearest positioned +
    overflow:hidden ancestor). This confines the modal to the app window only.
  - Sizing that used to be viewport-based (`80vh`) is now percentage-based, so it
    scales with the app window's own height instead of the browser viewport.
  - Spacing/type that needs to shrink as the window narrows now uses CSS
    container queries (`@container`) keyed off `.projects-app-root`, which has
    `container-type: inline-size`. This reacts to the app window's own width —
    even when the browser viewport itself doesn't change — which is what makes
    it adjust when you resize the app window on desktop/laptop.
*/
      ...
      >
        ...
        >
          ...
          <h2
            ...
            style={{ textShadow: `0 0 20px ${project.accentColor}` }}
          >
            {project.titleMain}
          </h2>
    ...
    <div className="projects-app-root">
      <style dangerouslySetInnerHTML={{
        ...
        .projects-app-root {
          ...
          /* Establishes this app window as a CSS container-query context, so
             descendants (like the project modal) can respond to the app
             window's own size instead of only the browser viewport. */
          container-type: inline-size;
          container-name: projectsApp;
        }
        ...
        /* ── Project Detail Modal ──────────────────────────────────────
           Positioned absolute (not fixed) so it's confined to this app
           window (.projects-app-root is position:relative + overflow:hidden)
           instead of covering the whole desktop. Sizing below reacts to the
           app window's own width via @container queries, and to its own
           height via percentages — so resizing the app window on desktop
           reflows the modal, not just real mobile/tablet viewports. */
        ...
        /* App window is medium-narrow (e.g. laptop window resized down) */
        @container projectsApp (max-width: 700px) {
          .project-modal-overlay { padding: 1.25rem; }
          .project-modal-box { max-height: 90%; border-radius: 16px; }
          .project-modal-header { padding: 1.25rem; }
          .project-modal-title { font-size: 1.3rem; }
          .project-modal-content { padding: 1.25rem; }
          .project-modal-footer { padding: 0.9rem 1.25rem; }
        }

        /* App window is quite narrow */
        @container projectsApp (max-width: 520px) {
          .project-modal-overlay { padding: 0.75rem; }
          .project-modal-box { max-height: 92%; border-radius: 14px; }
          .project-modal-header { padding: 1rem; }
          .project-modal-title { font-size: 1.1rem; padding-right: 1.75rem; }
          .project-modal-content { padding: 1rem; }
          .project-modal-footer { padding: 0.75rem 1rem; flex-direction: column; }
          .project-modal-footer a { justify-content: center; width: 100%; }
        }

        /* App window is very narrow */
        @container projectsApp (max-width: 380px) {
          .project-modal-overlay { padding: 0.5rem; }
          .project-modal-title { font-size: 1rem; }
          .project-modal-close { width: 28px; height: 28px; top: 0.75rem; right: 0.75rem; }
        }
      ` }} />
